...
from UniWeb.forms import (
    ApiUserRegistrationForm,
    CareersInternshipform,
    UpdateUserDetails,
)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if username and password:
                ...
                messages.success(request, "Updated Successfully. Log In to continue.")
...
def careers_view(request, *args, **kwargs):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            qualificationAndqueries = form.cleaned_data.get("qualificationAndqueries")
            try:
...
def student_details_chart(request, user):
    ...
    for sem in student_academic_list:
        ...
        for obj in Academic.objects.filter(
            student__user=user, semester__name=sem.semester.name
        ):
